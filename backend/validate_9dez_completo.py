"""
Validação completa para 9 de dezembro de 2025.
Verifica desde os cálculos até a lógica de agrupamento.
"""
import sys
import os
...
print("="*80)
print("VALIDAÇÃO COMPLETA - 9 DE DEZEMBRO DE 2025")
print("="*80)
print()

# Verificar todas as ações que podem ter Lua como benéfico
print("Ações disponíveis:")
for action_id, config in ACTION_HOUSES.items():
    if 'Lua' in config.get('beneficial_planets', []):
        print(f"\n{action_id}:")
        print(f"  Casas primárias: {config['primary_houses']}")
        print(f"  Casas secundárias: {config['secondary_houses']}")
        print(f"  Planetas benéficos: {config['beneficial_planets']}")
        print(f"  Aspectos preferidos: {config['preferred_aspects']}")

# Testar com primeiro_encontro (mais provável baseado nas casas)
action_id = 'primeiro_encontro'
action_config = ACTION_HOUSES[action_id]

print("\n" + "="*80)
print(f"TESTANDO AÇÃO: {action_id}")
print("="*80)
print()
...
# Planetas a verificar (Lua está na lista de benéficos)
planets_to_check = ['moon']
planet_names = {'moon': 'Lua'}
...
for asp in expected_aspects:
    if "Casa 5" in asp or "Casa 7" in asp or "Casa 1" in asp:  # Primárias
        if "trígono" in asp:
            expected_score += 10
        elif "sextil" in asp:
            expected_score += 7
    elif "Casa 11" in asp:  # Secundária
        if "sextil" in asp:
            expected_score += 3
...
# Agora testar com a função calculate_best_timing
print("\n" + "="*80)
...
moments_9dez = [m for m in result.get('best_moments', []) if '2025-12-09' in m['date']]

print(f"Momentos encontrados para 9/12/2025: {len(moments_9dez)}")
if moments_9dez:
    for m in moments_9dez:
        print(f"\n  {m['date']}:")
